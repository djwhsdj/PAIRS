    ...
    def prune_step(self, mask, closure=None):
        ...
        for group in self.param_groups:
            for name, p in group['params']:
                ...
                if name.split('.')[-1] == "weight" and len(p.shape) == 4 and 'downsample' not in name and p[0,:,:,:].shape == p[:,0,:,:].shape: ############# changed by me! ######
                    if name[:5] == 'conv1' :
                        continue
                ...
                if name.split('.')[-1] == "weight" and len(p.shape) == 4 and 'downsample' not in name and p[0,:,:,:].shape == p[:,0,:,:].shape: ########## changed here too ######
