...
def update_Z(X, U, pattern_set, withoc):
    new_Z = ()
    for x, u in zip(X, U):
        z = x + u
        
        # Select each kernel and prune -> z = torch.tensor (a, b, 3, 3) or (a, b, 1, 1)
        z = torch.from_numpy(top_4_pat(z.numpy(), pattern_set, withoc))
...
def prune_weight(weight, device, percent, pattern_set, withoc):
    ...
    weight_numpy = top_4_pat(weight_numpy, pattern_set, withoc)
    
    mask = torch.Tensor(weight_numpy != 0).to(device)
    return mask

def apply_prune_pat(args, model, device, pattern_set, pattern_set_1, pwr, withoc, method):
    ...
    for name, param in model.named_parameters():
        if name.split('.')[-1] == "weight" and len(param.shape) == 4 and 'downsample' not in name and param[0,:,:,:].shape == param[:,0,:,:].shape:
            ...
            if method == 'ours' :
                if pwr[idx] > 3 :
                    mask = prune_weight(param, device, args.connect_perc, pattern_set_1, withoc)
                else : 
                    mask = prune_weight(param, device, args.connect_perc, pattern_set, withoc)
            else :
                mask = prune_weight(param, device, args.connect_perc, pattern_set, withoc)
...
def apply_prune(args, model, device, pattern_set, withoc):
    # returns dictionary of non_zero_values' indices
    dict_mask = {}
    idx = -1
    for name, param in model.named_parameters():
        if name.split('.')[-1] == "weight" and len(param.shape) == 4 and 'downsample' not in name and param[0,:,:,:].shape == param[:,0,:,:].shape:
            if name[:5] == 'conv1' :
                continue
            idx += 1
            # mask = prune_weight(param, device, args.connect_perc, pattern_set[idx])
            mask = prune_weight(param, device, args.connect_perc, pattern_set[idx], withoc)
            param.data.mul_(mask)
